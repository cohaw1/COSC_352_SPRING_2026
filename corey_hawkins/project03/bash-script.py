...
run_and_test_container() {
    ...
    # Determine the path to the test files relative to the script's location
    ...
    # Run the container, mount the input file, and capture output

    # The command inside the container needs to be adapted based on how the student's application expects input.

    # This example assumes the application reads from stdin or a specific file path.

    # We'll use 'docker run --rm' to automatically remove the container after it exits.

    # We mount the input file to a known location inside the container.

    local container_input_path="/app/input.txt" # Example path inside container

    local container_output_path="/app/output.txt" # Example path for container to write output

    # Construct the command to run inside the container. This might need adjustment.

    # Example: If the student's app reads from stdin:
    ...
    # For simplicity, let's assume the app reads from stdin and we pipe the input to it.

    # The 'your_app_command' needs to be replaced with the actual command to run the student's program.

    # If the Dockerfile specifies a CMD or ENTRYPOINT, docker run might execute it directly.

    # We'll assume the Dockerfile's CMD/ENTRYPOINT is what we want to run.

    local actual_output=""

    local container_exit_code=0

    # Use a temporary file to capture output from the container

    local temp_output_file=$(mktemp)

    log_message "Running container $image_name and executing tests..."

    # The command below is a placeholder. You MUST adapt 'your_app_command'

    # and the way input/output is handled based on the student's project.

    # This example pipes the input file content to the container's stdin.
    ...
    if docker run --rm -v "$(pwd)/$relative_test_input_path:$container_input_path" \
        ...
        # Copy the output from the temporary file inside the container to the host
        ...
        # Attempt to capture any output that might have been written to stderr or stdout before failure
...
# Clear log file at the start of each grading run
...
find "$STUDENT_PROJECTS_DIR" -mindepth 1 -maxdepth 1 -type d | while read -r student_dir; do
    ...
    # Iterate through each project to grade
    ...
    for project_name in "${PROJECTS_TO_GRADE[@]}"; do
        ...
        if [ -d "$project_path" ]; then
            ...
            # Clean up the Docker image to save space (optional, but good practice)
